...
export const useGafpriPaginationsIncome = ({
  useData,
}: UseGafpriPaginationsIncomeProps): UseGafpriPaginationsIncomeReturn => {
  ...
  function sortByName(
    ...
    if (items) {
      return items.slice().sort((a, b) => {
        const comparison = a.incomeCustomer[0].name.localeCompare(
          b.incomeCustomer[0].name,
          undefined,
          {
            sensitivity: 'base',
          }
  ...
  const filterByName = (search: string): IncomeAttributes[] | null => {
    if (useData.states.items && useData.states.items.data.items) {
      return useData.states.items.data.items.filter((item) =>
        item.incomeCustomer[0].name.toLowerCase().includes(search.toLowerCase())
      );
    }
    return null;
  };
